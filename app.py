...
def get_gifs_from_giphy(search_string):
    ...
    # data = requests.get(baseurl + '?q=' + search_string + '&api_key=' + api_key + '&limit=5').json()
    params = {'q': search_string, 'api_key': api_key, 'limit': 5}
    data = requests.get(baseurl, params = params)
    res_d = json.loads(data.text)
    return res_d['data']
...
def get_or_create_collection(db_session, name, current_user, gif_list=[]):
    ...
    collection = db_session.query(PersonalGifCollection).filter_by(name=name, user_id=current_user.id).first()
    if collection:
        return collection
    else:
...
def index():
    ...
    form = GifSearchForm()
    if form.validate_on_submit():
        get_or_create_search_term(db.session, term=form.search.data)
...
def create_collection():
    ...
    if request.method == 'POST':
        ...
        print("GIFS RETURNED", gifs_objects)
        get_or_create_collection(db.session, name=form.name.data, current_user=current_user, gif_list=gifs_objects)
...
def collections():
    ...
    c = PersonalGifCollection.query.all()
    return render_template('collections.html', collections=c)
